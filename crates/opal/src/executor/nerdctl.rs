...
impl NerdctlExecutor {
    ...
    pub fn build_command(ctx: &EngineCommandContext<'_>) -> Command {
        build_container_engine_command("nerdctl", ctx)
    }
}

#[cfg(test)]
mod tests {
    use super::NerdctlExecutor;
    use crate::engine::EngineCommandContext;
    use std::path::Path;

    #[test]
    fn build_command_uses_nerdctl_binary() {
        ...
        let command = NerdctlExecutor::build_command(&ctx);

        assert_eq!(command.get_program().to_string_lossy(), "nerdctl");
    }
}
