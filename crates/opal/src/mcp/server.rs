...
fn initialize_result() -> Value {
    json!({
        ...
        "instructions": "Use the Opal MCP tools to plan runs, explain job planning decisions, execute pipelines, rerun recorded job names against the current checkout, inspect recorded history and logs, compare recent runs, search prior logs, list recent runs, and quickly identify failed jobs in the latest or a selected run."
    })
}
