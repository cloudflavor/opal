...
The MCP server currently exposes planning, run execution, run inspection, history listing with status, job, branch, pipeline-file, and date-range filters, failed-job lookup, run-diff, log-search, job-rerun, plan-explanation, and engine-status tools for agent workflows.
