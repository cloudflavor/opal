...
- **Artifact discipline** – Job outputs land under `~/.opal/<project>/<run>/artifacts/<job>/` and are shared read-only with downstream jobs that declare `needs: { artifacts: true }`. Host `target/` is never touched; the workspace stays clean.
...
└─ .opal/         # Optional repo-scoped config/secrets (runtime data lives in ~/.opal/<project>/…)
```
