...
- **Artifact discipline** – Job outputs land under `.opal/<run>/artifacts/<job>/` and are shared read-only with downstream jobs that declare `needs: { artifacts: true }`. Host `target/` is never touched; the workspace stays clean.
...
└─ .opal/         # Ephemeral run artifacts created at runtime
```
