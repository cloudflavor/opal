...
6. **Log pane** – Streams job output live. Scroll with `↑/↓`, `PgUp/PgDn`, `Ctrl+u/d`, `g/G`, etc. Press `o` to open the current log or AI analysis view in your pager. Press `a` to start AI analysis for the selected current-run job, and press `a` again to toggle between the normal log view and the saved/streamed analysis view. Press `A` to preview the exact rendered AI prompt that Opal will send, and press `A` again to close that preview.
